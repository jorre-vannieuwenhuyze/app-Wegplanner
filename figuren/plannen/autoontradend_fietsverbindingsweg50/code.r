makeresult <- function(path,breedte) {HTML(
  ...
  ,characteristics(
    ...
    ,'Markering'  = 'Een witte stippellijn om het snelle tweerichtings-verkeer te onderstrepen' 
    )
  ,voorbeeldplannen
  ,ifelse( 0.0 <= breedte & breedte <  3.5,toosmall,"")
  ,ifelse( 3.5 <= breedte & breedte <  5.0,plan(path,'plan1'),"")
  ,ifelse( 5.0 <= breedte & breedte <  7.0,plan(path,'plan2'),"")
  ,ifelse( 7.0 <= breedte & breedte <  Inf,plan(path,'plan3'),"")
  ,praktijkvoorbeelden
